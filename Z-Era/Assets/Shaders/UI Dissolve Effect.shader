Shader "Migration/UI Dissolve Effect"
{
	Properties
	{
		...
		_FillFloor("Force Fill Location", Range(0, 0.9)) = 0.4
	}
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				// 把噪声值域抬升到 [margin, 1]：Location 降到 _FillFloor 后整个画面
				// 强制全覆盖，Location=0 时所有像素完全不透明、无边缘染色，
				// Location=1 时仍然完全溶解。
				float margin = max(IN.effectFactor.w / 4, _FillFloor + IN.effectFactor2.w / 32);
				cutout = cutout * (1 - margin) + margin;
